...
#' @param limit_days The number of days to display on the x-axis. Default is 549 days (approximately 1.5 years).
...
#' @param limit_at_reference Logical. If TRUE, the upper limit will be exactly the reference date.
...
scale_x_date_limit_days <- function(name = waiver(),
                                    limit_days = 549, reference_date = NULL, limit_at_reference = FALSE,
                                    ...,
  ...
  limit_days_by_range <- function(limit_days, reference_date, limit_at_reference) {
    function(range) {
      ...
      if (!limit_at_reference) {
        return(c(upper_limit - lubridate::days(limit_days), range[2]))
      } else {
        return(c(upper_limit - lubridate::days(limit_days), upper_limit))
      }
    }
  }
  ...
  ggplot2::scale_x_date(
    name = name,
    limits = limit_days_by_range(limit_days, reference_date, limit_at_reference),
    ...,
    expand = expand,
    position = position
  )
...
#' @export
scale_x_date_limit_1.5years <- function(limit_days = 549, ...) {
  scale_x_date_limit_days(limit_days = limit_days, ...)
}
...
scale_x_datetime_limit_days <- function(name = waiver(),
  ...
  if (!is.null(reference_date)) {
    reference_date <- tryCatch(
      lubridate::as_datetime(reference_date),
      error = function(e) NA
    )
    # give warning if after conversion if NA
    if (is.na(reference_date)) {
  ...
  limit_days_by_range <- function(limit_days, reference_date, limit_at_reference) {
    function(range) {
      if (!is_empty(reference_date) & is.POSIXct(reference_date)) {
        upper_limit <- reference_date
      } else {
        upper_limit <- range[2]
      }
