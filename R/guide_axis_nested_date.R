...
#' @param regular_key Default is `"auto"`, which generates the nested axis based on the date labels and the seperator above.
#' This option can be used to provide you own specification for the nested key. See [legendry::key_standard()]
...
#' @param mode Processing mode for the guide. Default is `"simple"`.
#' @param pad_date Numeric value controlling the padding around date ranges.
#'   If `NULL` (default), automatically sets to 0.5 for "fence" type and
#'   0.25 for other types.
#' @param oob How to handle out-of-bounds values. Default is `"none"`.
#' @param ... Additional arguments passed to [legendry::guide_axis_nested()].
#'
#' @return A nested axis guide object that can be used with [ggplot2::guides()].
#'
